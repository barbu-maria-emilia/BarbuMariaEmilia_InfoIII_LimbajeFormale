...
class Interpreter:
    ...
    def compute_bin(self, left, op, right):
        ...
        if op.value == "=":
            if right_type == "VAR":
                right_type = self.data.read(right.value).type
                right_value = self.read_VAR(right.value)
            else:
                right_value = getattr(self, f"read_{right_type}")(right.value)

            value_token = Integer(right_value) if right_type == "INT" else Float(right_value)
            left.type = f"VAR({right_type})"
            self.data.write(left, value_token)
    ...
    def interpret(self, tree=None):
        ...
        if isinstance(tree, list) and all(isinstance(x, list) or isinstance(x, Reserved) or hasattr(x, 'type') for x in tree):
            result = None
            for stmt in tree:
